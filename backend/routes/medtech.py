"""MedTech finance routes — Income/Expenses entries with categories Retail /
Wholesale, Revenue Type (Direct/Recurring), and Generated By (employee
name). See DEPARTMENT_CONFIG['MedTech'] in models.py for the exact
field rules this file enforces.
"""
...
def list_entries():
    ...
    if search:
        like = f"%{search}%"
        query = query.filter(
            (FinanceEntry.generated_by.ilike(like)) | (FinanceEntry.remarks.ilike(like))
        )
