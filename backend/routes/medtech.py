"""MedTech finance routes — Income/Expenses entries with categories
Wholesale / Retail / B2C, Revenue Type, Generated By, Client Name, GST
Number, GST Tax (%), Tax Invoice Number, line items (product name/qty/
price), and an optional Invoice upload (PDF/image/Word/Excel doc).
See DEPARTMENT_CONFIG['MedTech'] in models.py.

GST Tax is a percentage applied on top of the item subtotal (or the plain
Amount, for departments that don't use line items): the amount actually
stored/reported (`amount`) is base_amount + gst_tax_amount, where
gst_tax_amount = base_amount * gst_tax_percent / 100. base_amount is kept
separately so the pre-tax subtotal is never lost.

Create/update use multipart/form-data so the invoice file can travel with
the rest of the fields; the `items` list is sent as a JSON string in the
"items" form field and parsed here.
"""
import json
...
def _parse_gst_tax_percent(raw_value):
    """Returns (percent, error). Empty/missing input is treated as 0% —
    GST Tax is optional; if left blank, the total is just the base amount."""
...
def create_entry():
    ...
    # base_amount = item subtotal; gst_tax_amount = base_amount * pct / 100;
    # amount (the figure used everywhere else in the app — stats, summary,
    # exports) = base_amount + gst_tax_amount.
...
def update_entry(entry_id):
    ...
    effective_category = data.get("category", entry.category)
    if effective_category in CONFIG["gst_required_categories"] and not entry.gst_number:
        errors.append(f"gst_number is required for {effective_category} entries.")

    # Recompute base_amount from new items if provided, otherwise fall back
    # to whatever base_amount is already stored (or, for entries created
    # before this feature existed, the old `amount` value as a best guess).
    ...
    # Always keep amount/base_amount/gst_tax_amount consistent with each
    # other, whether items or the tax rate (or neither) changed.
    gst_tax_amount = round(base_amount * gst_tax_percent / 100, 2)
