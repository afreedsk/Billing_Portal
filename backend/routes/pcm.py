...
Taker, Nurse, RMP, Physico Care, plus a free-text "Others" category (via
category="Others" + other_category="<custom name>", resolved server-side
into the actual stored category). Also has Patient Name / Patient Place.
No Revenue Type or Generated By field for this department. See
...
def _resolve_category(submitted_category, other_category, allowed_categories):
    """Returns (final_category, error). If the person picked "Others", the
    final category is whatever custom text they typed in other_category;
    otherwise the submitted category must be one of the fixed options."""
    if submitted_category == "Others":
        custom = (other_category or "").strip()
        if not custom:
            return None, "Please enter a category name for 'Others'."
        return custom, None

    if submitted_category not in allowed_categories:
        return None, f"category must be one of: {', '.join(allowed_categories)}."

    return submitted_category, None
...
def create_entry():
    ...
    category, category_error = _resolve_category(submitted_category, other_category, allowed_categories)
    if category_error:
...
def update_entry(entry_id):
    ...
    if "category" in data:
        category, category_error = _resolve_category(
            data.get("category"), data.get("other_category"), allowed_categories
        )
        if category_error:
            errors.append(category_error)
        else:
